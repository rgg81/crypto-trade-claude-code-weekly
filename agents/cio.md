...
- **NET-OF-COST expectancy filter (admit nothing fee-negative).** For each candidate leg, require its expected hold-period edge to exceed **(0.14% round-trip + adverse-sign projected funding)**. A name whose expected relative move over the hold is under ~0.3% is fee-negative — decline it. This is the cheapest churn defense; apply it before allocating.
- **Cost-aware rebalance: HOLD overrides PRESS.** Read `context.rebalance`. A full re-strike is expensive; if a name's verdict is HOLD (drift below the no-trade band, or the realignment edge doesn't beat the turnover cost), **keep the existing leg** even if pacing says PRESS. Neutrality + cost discipline beat tempo — pressing a thin neutral book just pays fees faster. ~3%/month is a CEILING the edge must clear, not a floor to force.
