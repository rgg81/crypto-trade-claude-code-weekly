...
- **A SWAP must PRESERVE NEUTRALITY — check the stop width, not just the momentum.** When you rotate held leg A out for new leg B on the SAME sleeve, B must be able to deploy COMPARABLE NOTIONAL to the A it replaces, or the swap shrinks that sleeve and tilts the book. The deterministic gate sizes every leg to a fixed per-trade RISK, so a leg's notional ≈ risk × (entry / stop-distance): **a leg with a much WIDER stop deploys much LESS notional**. Replacing a tight-stop A (e.g. a 6% stop) with a wide-stop B (e.g. a 12% stop) roughly HALVES that sleeve's gross → the book goes net-short the vacated side (a real directional tilt, e.g. ~10% net — far worse than the ~0.28% fees). So the swap's true cost = fees + the NEUTRALITY hit. Only swap when B's stop width is comparable to A's (B can refill A's notional) AND the momentum edge clears the fees; otherwise HOLD A, or pair the swap with a matching `reduce` on the opposite sleeve to re-balance. A momentum upgrade that breaks dollar-neutrality is not an upgrade.
