"""Single-flight run lock for the dual-loop serialized runner (TEMPEST-WEEKLY).

The fast (15m) and strategic (1h) loops share ONE paper account/book. There is no concurrency in
the engine; correctness depends on EXACTLY ONE writer at a time. Atomic state writes prevent torn
files but NOT lost updates (load->mutate->save races). This lock closes that gap: every poll fire
tries to acquire `state/.run.lock` before running any loop; if it is held by a live run, the fire
stands down (the next poll retries). A stale lock — a crashed run that never released — is
reclaimed after `stale_after_s`, so a crash can never wedge the desk permanently.
...
LOCK_NAME = ".run.lock"
DEFAULT_STALE_AFTER_S = 1800.0  # 30 min: well beyond a normal strategic cycle, short enough to heal
...
def try_acquire(state_dir, now: datetime, *, owner: str = "runner",
    ...
    create and returns True — the losers re-observe the fresh holder and return False. (The previous
    blind os.replace overwrite let two processes both 'reclaim' the same stale lock and both proceed
...
def single_flight(state_dir, now: datetime, *, owner: str = "runner",
    ...
    """Context manager: yields True if the lock was acquired (releasing on exit), else False.
        ...
        with single_flight(state_dir, now) as ok:
            if not ok:
                return  # another loop is running; stand down
            ... run the loop ...
    """
